...
class ConfigWindowController: NSWindowController {
    ...
    private func createAPITabView() -> NSView {
        ...
        apiProviderPopup = NSPopUpButton(frame: NSRect(x: 150, y: 415, width: 200, height: 28), pullsDown: false)
        ...
        youdaoAppSecretField = NSSecureTextField(frame: NSRect(x: 130, y: 290, width: 300, height: 24))
        youdaoAppSecretField.placeholderString = "输入有道 App Secret"
        view.addSubview(youdaoAppSecretField)
        
        let youdaoHelpButton = NSButton(title: "获取密钥 →", target: self, action: #selector(openYoudaoHelp))
        ...
        let googleHelpButton = NSButton(title: "获取密钥 →", target: self, action: #selector(openGoogleHelp))
        ...
        let bingHelpButton = NSButton(title: "获取密钥 →", target: self, action: #selector(openBingHelp))
        ...
        // 保存按钮
        let saveButton = NSButton(title: "保存 API 设置", target: self, action: #selector(saveAPISettings))
    ...
    private func createDisplayTabView() -> NSView {
        ...
        let autoSpeakCheckbox = NSButton(checkboxWithTitle: "自动朗读单词", target: self, action: #selector(autoSpeakChanged(_:)))
        ...
        let voicePopup = NSPopUpButton(frame: NSRect(x: 130, y: 310, width: 150, height: 28), pullsDown: false)
        ...
        let cacheLabel = NSTextField(labelWithString: "缓存单词: \(cacheStats.count) 个，占用: \(ByteCountFormatter.string(fromByteCount: cacheStats.size, countStyle: .file))")
        ...
        let resetProgressButton = NSButton(title: "重置学习进度", target: self, action: #selector(resetLearningProgress))
        ...
        let resetAllButton = NSButton(title: "恢复默认设置", target: self, action: #selector(resetAllSettings))
    ...
    @objc private func openGoogleHelp() {
        NSWorkspace.shared.open(URL(string: "https://console.cloud.google.com/apis/library/translate.googleapis.com")!)
    }
    
    @objc private func openBingHelp() {
        NSWorkspace.shared.open(URL(string: "https://portal.azure.com/#create/Microsoft.CognitiveServicesTextTranslation")!)
    }
        ...
        Task {
            do {
                ...
                DispatchQueue.main.async {
                    ...
                    self.loadData()
                    self.showAlert(title: "下载完成", message: "\(source.name) 下载成功！", style: .informational)
                }
            } catch {
                DispatchQueue.main.async {
                    sender.isEnabled = true
        ...
        Task {
            do {
                ...
                DispatchQueue.main.async {
                    ...
                    self.showAlert(title: "设置成功", message: "已切换到 \(source.name)", style: .informational)
                }
            } catch {
                DispatchQueue.main.async {
                    self.showAlert(title: "设置失败", message: error.localizedDescription, style: .warning)
                }
            }
        }
    }
